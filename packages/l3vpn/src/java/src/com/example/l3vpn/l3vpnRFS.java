...
public class l3vpnRFS {
  private static Logger LOGGER = Logger.getLogger(l3vpnRFS.class);
    ...
    throws NavuException
  {
    NavuContainer topology = root.getParent().container(l3vpn.uri)
      .container(l3vpn._topology);
    NavuLeaf devicesLeaf = topology.list(l3vpn._role).elem(roleName)
      .leaf(l3vpn._device);
  ...
  public static List<String> getCEonVPE(NavuNode ncsRoot) throws NavuException {
    ...
    ConfList onvpe = (ConfList) ncsRoot.getParent().container(l3vpn.uri)
      .leaf(l3vpn._onvpe).value();
